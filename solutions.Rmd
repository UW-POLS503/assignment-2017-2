...
```{r}
diff.reg <- lm(log(pauper2) - log(lag(pauper2)) ~ I(log(outratio)-log(lag(outratio))) + I(log(Prop65) - log(lag(Prop65))) + I(log(Popn2) - log(lag(Popn2)) + (log(Popn2) - log(lag(Popn2)))), data=pauperism)
...
Autocorrelation is certainly present in this data and M2 will do a better job of controlling for autocorrelation, I believe. I think that autocorrelation will be controlled more in M2 because M2 does not treat data points on the same ID as independent in different years. For this reason, I would choose M2 over M5. However, autocorrelation will only affect uncertainty, not bias. Perhaps M5 is more biased since it treats each point as independent in the different years, while M2 does not, though I'm not exactly sure if/how that would bias the estimated coefficient.
...
```{r}
beta_R / (beta_R - beta_F)
# [1] 3.159196
```

## Calculate this statistic for *M2* and interpret it.

The statistic is 3.16. The statistic is mean to gauge, relative to the included control variables, the required effect of omitted variable bias to make the observed coefficient 0. $\hat\beta_F$ denotes the coefficient in the full model and $\hat\beta_F /(\hat\beta_R - \hat\beta_F)$ denotes the change in the coefficient due to the control variables. So, relative any omitted variables would have to have a 3.16x larger effect than the current set of control variables for the estimated coefficient to be 0. I think this metric could made more useful if, instead of looking at the required effect of the potential omitted variables to make the coefficient 0, the metric looked at the required effect of the potential omitted variables to make the coefficient substantively null.
...
```
```{r}
sqrt(diag(vcov(M3.reg)))
```
The outratiodiff coefficients remain unchanged in the heteroskedacticity consistent and non-heteroskedacticity-consistent errors. Since the general lm command does not account for heteroskedasticity, it must be that the errors in M2 and M3 are homoskedastic enough that the estimates remain unchanged whether using HAC or not. Looking at the plot below, I think the data looks fairly homoskedastic (citation for the code used to produce the spread-level plot: "Ression Diagnostics." Statmethods.net. Robert I. Kabacoff, Ph.D, n.d. Web. 2 May 2017. <http://www.statmethods.net/stats/rdiagnostics.html>.)


```{r}
library(car)
spreadLevelPlot(M2.reg)
```
...
```
The coefficient on outratiodiff varies markedly between models (ranges from 0.7107 where Type is Ubran and year is 1881 to .1657 where Type is Mixed and year is 1891). For each type, the coefficient is higher in 1881 than it is in 1891. Similaryly, the standard error for outratiodiff and the residual standard errors are higher in 1891 than in 1881 within each type.

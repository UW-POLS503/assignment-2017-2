...
```{r}
restricted3 <- lm(paupratiodiff ~ outratiodiff + (popratiodiff + oldratiodiff) * (year + Type), data=pauperism)
...
# re-do this section, to use lag for lag variables and I() for differences
...
M5.reg <- lm(I((log(pauper2t) -lag(log(pauper2t_minus_1))) ~ I(log(outratio_t)-log(outratio_t_minus_1)) + I(log(Prop65_t) - log(Prop65_t_minus_1)) + I(log(Popn2_t) - log(Popn2_t)) + log(Popn2_t) - log(Popn2_t), data=M5data)
...
# Why does logging help substantively in this  example?
...
# Dealer choice on which you want to compare this model to
